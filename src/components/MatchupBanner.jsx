...
function Side({ Photo, photoProp, name, sport, winner }) {
...
export default function MatchupBanner({ sport, nameA, nameB, participantType, winner = null }) {
  ...
  return (
    <div className="matchup-banner">
      <Side Photo={Photo} photoProp={photoProp} name={nameA} sport={sport} winner={winner} />
      <span className="matchup-banner-vs">
        <SportIcon sport={sport} size={16} />
        VS
      </span>
      <Side Photo={Photo} photoProp={photoProp} name={nameB} sport={sport} winner={winner} />
    </div>
  )
}
