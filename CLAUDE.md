...
| `odds-snapshot.js` | `*/30 * * * *` | snapshots prices for open-bet legs (CLV) and followed fixtures (sharp-money); also posts Nova's biggest sharp-money move to Discord (`src/lib/marketSteam.js`) |
...
| `sage-propose.js` | `0 8 * * *` | Sage - the single research/ideas agent - proposes one fact-checked idea grounded in BOTH the web (Claude + `web_search`) AND the site (real usage/feedback/gaps gathered from the DB, see `gatherSiteContext`), posts it to Discord with Approve/Reject buttons; approve optionally opens a GitHub issue (no-ops without the Anthropic key + Discord bot - see `docs/sage-agent-setup.md`) |
| `community-pulse.js` | `0 10 * * *` | Bea posts a Discord line when a group crosses a member-count milestone (`src/lib/communityMilestone.js`); dedupe watermark `groups.last_member_milestone` (no-ops without `DISCORD_WEBHOOK_URL`) |
