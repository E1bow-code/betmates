...
| `matchday-brief.js` | `0 7 * * *` | the research desk (Jonas/Rue/Vic/Ola/Finn) posts one fact-checked, cited pre-match brief (form/weather/injuries/refs/travel) for the soonest upcoming followed fixture via Claude + `web_search`; context, never a tip; watermarked by `followed_fixtures.brief_sent_at` (no-ops without the Anthropic key + `DISCORD_WEBHOOK_URL` - see `docs/agent-signals.md`) |
